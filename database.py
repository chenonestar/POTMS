...
# destination_passport 是自由文本。这一栏当年叫「地点、证照」，要求把目的地和
# 证件类型写在一起（如「美国-护照」「香港/港澳通行证」），本函数就是从那半句
# 证件类型里认字——所以它只认**证件名称**，不认地名：「香港」既可能持港澳通行证
# 也可能持护照过境，拿地名猜会猜错。
#
# 这一栏现已收窄为只填地点、改名「目的地」，本级判据因此只对**存量**数据有意义：
# 新记录的证件种类走结构化的 intended_cert_type，不再需要从文字里猜。留着是因为
# 老库里那半句话还在，回填时仍要认它。
