...
签章时间 `signed_at` 在**签名载荷之内**，因此改库改不动它：动了时间，签名当场验不过。

但要说清楚它证明了什么、没证明什么：

| | |
|---|---|
| ✅ 能证明 | 这份凭证自签章后没被动过，**且签章时本机声称是这个时间** |
| ❌ 不能证明 | 这个时间是真的 |

时间取自**本机时钟**。有管理员权限的人可以把系统时间往前调再签章，签出来的章一样
有效——这是自述时间的固有边界，不是实现缺陷。要拿到不可否认的时间，只能引第三方
RFC 3161 时间戳服务（TSA），而那需要网络可达且对方认可该 TSA，内网部署基本不具备
条件；何况默认证书本身还是自签的（「只防内部篡改、不能对外举证」），
单独去接 TSA 属于给帐篷装金库门。

若将来换成 CA 证书对外举证，再补 TSA 才有意义，接入点是 `Sm2Seal.sign()` 里取
`signedAt` 那一处。

> **历史存证**：本改动之前生成的签章，时间在载荷之外。这类章仍能验过，但详情页会
> 标成**「签章有效（旧版，时间未受保护）」**并用警示色，不与新章混作一谈。
> 需要升格的话，只能作废后重新登记——签名一经保存不可编辑，这是有意的。
...
```
POTMS_REQUIRE_SIGNATURE=0
```

取值 `0` / `false` / `no` / `off` 放宽，其余（含不设置）一律强制。

放宽后：签名板照常显示，只是标注「可留空」，配了手写板直接就能用；
**只放宽「可以不签」，不放宽「可以乱签」**——签了名仍要过格式校验
（PNG 魔数、大小上限），不合法照样拒绝。
...
404 / 500 / 403 都渲染中文错误页（版式与 Python 版一致），而不是框架自带的
英文默认页。错误页不依赖数据库、会话与布局——它最需要的就是在别的东西都坏掉时
仍能渲染出来。
