...
| 文件 | 内容 |
|---|---|
| **`系统需求与分析文档.html`** | **现行需求与系统分析的唯一依据；业务口径以此为准。含状态机、跨表影响矩阵、异常与回退路径，并可作为下一个项目的需求分析模板** |
| `开发需求文档.html` | 第一版需求文档，**已停止维护，仅作历史参照**。刻意保留：《开发经验教训总结》第二节逐句引用它来论证返工的成因 |
| `系统评审报告.html` | 首轮评审发现的问题清单 |
| `系统优化审定报告.html` | 16 项优化点的审定与落实记录 |
| **`开发经验教训总结.html`** | **两个月迭代下来的经验与教训，含一份可逐条对照的检查清单。动手写新功能前值得先翻一遍** |
